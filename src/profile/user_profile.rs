use crate::{
    ...
    sliding_sync::{get_client, is_user_ignored, submit_async_request, MatrixRequest},
    utils,
};
use makepad_widgets::*;
use matrix_sdk::{
...
use super::user_profile_cache::{
    self, get_user_profile_and_room_member, get_user_room_member_info,
};
...
impl Widget for UserProfileSlidingPane {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Signal = event {
            ...
            if let Some(our_info) = self.info.as_mut() {
                if let (Some(new_profile), room_member) =
                    get_user_profile_and_room_member(cx, &our_info.user_id, &our_info.room_id)
                {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        info.avatar_state
            ...
            .and_then(|data| {
                avatar_ref
                    .show_image(None, |img| utils::load_png_or_jpg(&img, cx, &data))
                    .ok()
            })
        ...
        self.label(id!(membership_title_label))
            .set_text(&info.membership_title());
        self.label(id!(membership_status_label))
            .set_text(info.membership_status());
        self.label(id!(role_info_label))
            .set_text(info.role_in_room().as_ref());
        ...
        let is_pane_showing_current_account = info
            ...
            .unwrap_or_else(|| {
                get_client().map_or(false, |client| client.user_id() == Some(&info.user_id))
            });
        ...
        ignore_user_button
            .set_enabled(!is_pane_showing_current_account && info.room_member.is_some());
        ...
        ignore_user_button.set_text(if is_ignored {
...
impl UserProfileSlidingPane {
    ...
    pub fn set_info(&mut self, _cx: &mut Cx, mut info: UserProfilePaneInfo) {
        if info.room_member.is_none() {
            if let Some(room_member) = get_user_room_member_info(_cx, &info.user_id, &info.room_id)
            {
        ...
        if info.room_member.is_none() {
            log!(
                "Did not find user {} room member info in cache, fetching from server.",
                info.user_id
            );
