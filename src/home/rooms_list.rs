...
impl RoomsListRef {
    ...
    pub(crate) fn bot_room_snapshots(&self) -> Vec<BotRoomSnapshot> {
        ...
        inner
            .all_joined_rooms
            .iter()
            .map(|(room_id, joined_room)| {
                let mut space_ids = BTreeSet::new();
                for (space_id, space_value) in &inner.space_map {
                    if space_value.direct_child_rooms.contains(room_id) {
                        space_ids.insert(space_id.clone());
                        for ancestor in &space_value.parent_chain {
                            space_ids.insert(ancestor.clone());
                        }
                    }
                }
                ...
                BotRoomSnapshot {
                    ...
                    space_ids: space_ids.into_iter().collect(),
                }
            })
            .collect()
    }
