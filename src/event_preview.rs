...
pub fn text_preview_of_timeline_item(
    ...
    match content {
        ...
        TimelineItemContent::MembershipChange(membership_change) => {
            text_preview_of_room_membership_change(membership_change).unwrap_or_else(|| {
                TextPreview::from((
        ...
        TimelineItemContent::OtherState(other_state) => text_preview_of_other_state(other_state)
            .unwrap_or_else(|| {
                TextPreview::from((
...
pub fn text_preview_of_message(message: &timeline::Message, sender_username: &str) -> TextPreview {
    let text = match message.msgtype() {
        ...
        MessageType::Location(location) => format!("[Location]: <i>{}</i>", location.body,),
        ...
        MessageType::Text(text) => text
            ...
            .as_ref()
            .and_then(|fb| {
                (fb.format == MessageFormat::Html).then(|| utils::linkify(&fb.body).to_string())
            })
        ...
        MessageType::VerificationRequest(verification) => format!(
            ...
            verification.from_device, verification.to,
        ),
        ...
        other => format!("[Unknown message type]: {}", other.body(),),
    };
...
pub fn text_preview_of_redacted_message(
    ...
    let text = match redactor_and_reason {
        Some((redactor, Some(reason))) => {
            ...
            format!(
                "{} deleted {}'s message: {:?}.",
                redactor, original_sender, reason
            )
        }
        Some((redactor, None)) => {
...
pub fn text_preview_of_other_state(other_state: &timeline::OtherState) -> Option<TextPreview> {
    let text = match other_state.content() {
        ...
        AnyOtherFullStateEventContent::RoomJoinRules(FullStateEventContent::Original {
            ...
            JoinRule::Knock => {
                format!("set this room to be joinable by invite only or by request.")
            }
            JoinRule::Private => format!("set this room to be private."),
            JoinRule::Restricted(_) => {
                format!("set this room to be joinable by invite only or with restrictions.")
            }
            JoinRule::KnockRestricted(_) => format!(
        ...
        AnyOtherFullStateEventContent::SpaceParent(_) => Some(format!(
            "set this room's parent space to {}.",
            other_state.state_key()
        )),
        AnyOtherFullStateEventContent::SpaceChild(_) => Some(format!(
            "added a new child to this space: {}.",
            other_state.state_key()
        )),
        _other => {
...
pub fn text_preview_of_room_membership_change(
    ...
    let text = match change.change() {
        ...
        Some(MembershipChange::Joined) => format!("joined this room."),
        Some(MembershipChange::Left) => format!("left this room."),
        ...
        Some(MembershipChange::Knocked) => format!("requested to join this room."),
