...
use crate::{
    ...
    }, room::{FetchedRoomAvatar, FetchedRoomPreview, RoomAliasResolveAction, RoomPreviewAction}, shared::{
        ...
        match request {
            ...
            MatrixRequest::ResolveRoomAlias(room_alias) => {
                ...
                let _resolve_task = Handle::current().spawn(async move {
                    ...
                    match res {
                        Ok(response) => {
                            let room_alias_id = room_alias.clone();
                            let room_id = response.room_id.clone();
                            let servers = response.servers.clone();
                            log!("Successfully resolved room alias {room_alias} to room ID {room_id}.");
                            Cx::post_action(RoomAliasResolveAction::Resolved {
                                room_alias_id,
                                room_id,
                                servers,
                            });
                        ...
                        Err(e) => {
                            log!("Failed to resolve room alias {room_alias}: {e:?}");
                            Cx::post_action(RoomAliasResolveAction::Failed {
                                room_alias_id: room_alias.clone(),
                                error: e.into(),
                            });
                        }
                    }
