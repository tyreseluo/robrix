...
<!-- screenshot: login-homeserver.png — Robrix 登录界面，底部的 Homeserver URL 输入框高亮显示 -->
...
> **提示：** 如果你使用了不同的 `server_name` 进行部署，请相应替换机器人 ID 中的 `127.0.0.1:8128`。例如，如果服务器的 `server_name = chat.example.com`，则机器人 ID 为 `@octosbot:chat.example.com`。
